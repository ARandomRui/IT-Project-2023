...
from ordermenu import app, db, bcrypt, mail
from ordermenu.forms import (RegistrationForm, LoginForm, UpdateAccountForm,
                             PostForm, RequestResetForm, ResetPasswordForm, AddfoodForm)
from ordermenu.models import User, Post, Food
...
def new_food():
    ...
    if form.validate_on_submit():
        if form.picture.data:
            image_type = "Food"
            picture_file = save_picture(form.picture.data, image_type)
            food = Food(name=form.name.data, price=form.price.data, description = form.description.data, image_file = picture_file)
        else:
            food = Food(name=form.name.data, price=form.price.data, description = form.description.data)
        db.session.add(food)
        db.session.commit()
        flash(f'Food, { form.name.data } has been successfully created!', 'success')
        return redirect(url_for('login'))
    return render_template('addfood.html', title='Register', form=form)

@app.route("/menu")
def menu():
    foodpage = request.args.get('page', 1, type=int)
    foodlist = Food.query.order_by(Food.id).paginate(page=foodpage, per_page=12)
    return render_template('menu.html', menulists=foodlist)
